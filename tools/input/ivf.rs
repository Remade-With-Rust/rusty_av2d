...
use rusty_av2d::include::dav1d::data::Dav1dData;
use rusty_av2d::{dav1d_data_create, dav1d_data_unref};
...
unsafe extern "C" fn ivf_probe(data: *const u8) -> c_int {
    // DKIF magic + the AV1 (AV01) or AV2 (AV02) codec FourCC. We accept both so the
    // same IVF demuxer feeds either codec into the decoder (rav2d targets AV2).
    let d = *(data as *const [u8; 12]);
    (d[0..4] == b"DKIF"[..] && (d[8..12] == b"AV01"[..] || d[8..12] == b"AV02"[..])) as c_int
}
...
unsafe extern "C" fn ivf_open(
    ...
    if ((*c).f).is_null() {
        ...
        if fread(hdr.as_mut_ptr() as *mut c_void, 32, 1, (*c).f) != 1 {
            ...
            if hdr[..dkif.len()] != dkif[..] {
                ...
                // Accept AV1 (AV01) or AV2 (AV02) streams.
                if hdr[8..12] != b"AV01"[..] && hdr[8..12] != b"AV02"[..] {
                    fprintf(
                        stderr(),
                        b"%s is not an AV1/AV2 file [tag=%.4s|0x%02x%02x%02x%02x]\n\0" as *const u8
                            as *const c_char,
                        file,
    ...
    *num_frames = 0 as c_int as c_uint;
    ...
    // HARDENING: a packet-less IVF (header only) has num_frames == 0 — avoid div-by-zero.
    (*c).step = if *num_frames != 0 { duration.wrapping_div(*num_frames) as u64 } else { 0 };
    fseeko((*c).f, 32, 0);
