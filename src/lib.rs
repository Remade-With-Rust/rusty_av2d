...
mod cdef;
mod cdef_apply;
#[allow(dead_code)] // AV2 coefficient decode entry (decode_coefs EOB) — live MSAC + generated coef CDFs
mod av2_coef;
#[allow(dead_code)] // AV2 decode-body primitives (partition enum + context fns); consumed by the recursion as it lands
mod av2_decode;
#[allow(dead_code)] // AV2 real decode_b neighbour-context machinery (a/l arrays + gather_nb + splats)
mod av2_recon;
#[allow(dead_code)] // AV2 MV reference-list scan (refmvs_find, brick B) — verified primitives + grid
mod av2_refmvs;
#[allow(dead_code)] // AV2 dequantizer (dq_lookup + per-coeff dequant + QM); bridges coeff tokens to itx input
mod av2_dequant;
#[allow(dead_code)] // AV2 in-loop filter primitives (CDEF constrain/directions + CCSO classifier)
mod av2_filter;
#[allow(dead_code)] // AV2 GDF (guided deblocking filter) — the learned NN filter + driver
mod av2_gdf;
mod av2_grain;
mod av2_grain_tables;
pub mod av2_palette;
pub mod av2_qm;
mod av2_qm_tables;
pub mod av2_lr;
pub mod av2_wedge;
pub mod av2_lr_tables;
#[allow(dead_code)] // GDF intra filter tables (auto-extracted from avm gdf_block.c)
mod gdf_tables;
#[allow(dead_code)] // GDF inter filter tables (auto-extracted from avm gdf_block.c)
mod gdf_inter_tables;
#[allow(dead_code)] // AV2 deblocking filter (filter_choice width selection + tapered boundary delta)
mod av2_deblock;
#[allow(dead_code)] // AV2 intra predictors (DC/V/H/Paeth/smooth); wired into reconstruction as it lands
mod av2_ipred;
#[allow(dead_code)] // AV2 1-D inverse transforms (DCT/ADST/DDT/identity); wired into the 2-D itx as recon lands
mod av2_itx;
#[allow(dead_code)] // AV2 reconstruction frame buffer + intra-edge gathering (recon pass)
mod av2_frame;
#[allow(dead_code)] // AV2 inter prediction (Stage D): motion compensation
mod av2_inter;
#[allow(dead_code)] // AV2 warped-motion compensation (Stage D)
mod av2_warp;
#[allow(dead_code)] // AV2 secondary transform (STX); wired into recon before the primary inverse transform
mod av2_stx;
mod av2_stx_tables;
#[allow(dead_code)] // AV2 transform-type (txtp) encoding + UV-mode LUT; drives the coeff decode + itx dispatch
mod av2_txtp;
#[allow(dead_code)] // GENERATED: STX basis kernels + DIP weights (tools/gen_tables.py), bit-exact from dav2d
mod av2_tables_gen;
mod cdf;
#[allow(dead_code)] // AV2 CDF defaults (generated + spec-validated); wired into the MSAC as block decode lands
mod cdf_av2;
mod const_fn;
pub mod cpu;
mod ctx;
...
fn gen_picture(c: &Rav1dContext, state: &mut Rav1dState) -> Rav1dResult {
    ...
    // AV2 POC-ordered output: drain one pending queued picture before parsing more input.
    if crate::obu::av2_pop_pending(c, state)? {
        return Ok(());
    }
    ...
    while !r#in.is_empty() {
        ...
        if let Ok(len) = len {
            // HARDENING: a corrupt OBU size field can make the parser report more bytes
            // consumed than remain — the slice would panic (start > end). Clamp to the tail.
            r#in.slice_in_place(len.min(r#in.len())..);
        }
