...
// NOTE: AV1-named C-ABI constants remapped to the nearest AV2 OBU type (the AV1
// FRAME/FRAME_HDR/REDUNDANT_FRAME_HDR/PADDING types don't exist in AV2).
pub const DAV1D_OBU_PADDING: Dav1dObuType = Rav1dObuType::BufRmTiming as Dav1dObuType;
pub const DAV1D_OBU_REDUNDANT_FRAME_HDR: Dav1dObuType =
    Rav1dObuType::MultiFrameHdr as Dav1dObuType;
pub const DAV1D_OBU_FRAME: Dav1dObuType = Rav1dObuType::ClosedLoopKf as Dav1dObuType;
...
pub const DAV1D_OBU_FRAME_HDR: Dav1dObuType = Rav1dObuType::MultiFrameHdr as Dav1dObuType;
...
pub enum Rav1dObuType {
    // AV2 OBU types (dav2d include/dav2d/headers.h). Note these renumber/replace AV1's.
    SeqHdr = 1,
    Td = 2,
    MultiFrameHdr = 3,
    ClosedLoopKf = 4,
    ...
    Bridge = 19,
    Msdo = 20,
    /// Quantization-matrix set OBU (avm OBU_QUANTIZATION_MATRIX).
    Qm = 22,
    /// Film-grain model tables OBU (dav2d DAV2D_OBU_FGM).
    Fgm = 23,
}

impl Rav1dObuType {
    /// AV2 OBU types that carry a frame (header and/or tile data).
    ...
    /// AV2 "show existing frame" style OBUs (no leading first_tile bit).
    pub fn is_sef_like(self) -> bool {
        use Rav1dObuType::*;
        matches!(self, Sef | Tip | Bridge)
    }
}
...
/// AV2-specific sequence-header fields (ones AV1 lacked), captured during
/// `parse_seq_hdr` and consumed by the frame header + decode path. Grown as needed.
#[derive(Clone, Copy, Default)]
#[repr(C)]
pub struct Av2SeqHdr {
    pub id: c_uint,
    pub ref_frames: u8,
    pub ref_frames_log2: u8,
    /// Inter-frame header gates (dav2d seq hdr): whether ref frames are signalled
    /// explicitly (else implicit scoring), and whether the refresh-flag uses the short form.
    pub explicit_ref_frame_map: u8,
    pub short_refresh_frame_flags: u8,
    /// Loopfilter sub-PU deblocking present (dav2d seq `db_sub_pu`); inter frames read an
    /// extra `lf_sub_pu` bit at the start of the deblock block when set.
    pub db_sub_pu: u8,
    ...
    pub order_hint_n_bits: u8,
    pub tip: u8,
    pub screen_content_tools: u8, // 0=off, 1=on, 2=adaptive
    pub force_integer_mv: u8,     // 0=off, 1=on, 2=adaptive
    pub def_max_bvp_drl_bits: u8,
    pub allow_max_bvp_drl_bits: u8,
    pub tiling_present: u8,  // 0=none, 1=present, 2=adaptive
    pub tiling_log2_cols: u8,
    pub tiling_log2_rows: u8,
    pub avg_cdf_type: u8,
    pub level: u8,
    pub tier: u8,
    pub ydc_dq_enabled: u8,
    pub uvdc_dq_enabled: u8,
    pub uvac_dq_enabled: u8,
    pub separate_uv_delta_q: u8,
    pub seg_info_present: u8,
    pub seg_adaptive: u8,
    pub seg_ext: u8,
    pub tcq: u8,
    ...
    pub df_par_bits: u8,
    pub gdf: u8,
    pub cdef_on_skiptx: u8, // 0/1/2 (2=adaptive)
    pub ccso: u8,
}
